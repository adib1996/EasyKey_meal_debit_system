...
if(isset($_POST['submit'])){
  ...
  if(empty($monthselected) && empty($dayselected)){ //Not a month nor a day is selected
    ...
    exit();
      ...
      WHERE MONTH(orders.order_date) = $monthselected AND orders.order_status = '1'
...
INNER JOIN retailer_info ON orders.retailer_id = retailer_info.retailer_id
INNER JOIN users ON orders.user_id = users.user_id
WHERE MONTH(orders.order_date) = $monthselected AND DAY(orders.order_date) = $dayselected AND orders.order_status = '1'";
