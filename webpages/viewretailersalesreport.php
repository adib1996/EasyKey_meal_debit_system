...
?>
  <div id="bodyindex">
    ...
    <form name="retailersales" target="_blank" action="generateretailersalesreportadmin.php" method="POST">

    <h3>Select Retailer</h3>

    <select name="retailerid" required>Select Retailer
    <option value="">- Select Retailer -</option>


    <?php

    $sql = "SELECT retailer_name, retailer_id FROM retailer_info";
    $result = mysqli_query($conn, $sql);
    $rescheck = mysqli_num_rows($result);
    if($rescheck < 1){
      echo '<script>alert("No Retailers Present as of Now \\nClick to Return to Homepage");
    window.location.href = "/index.php?viewreport=noretailers";
    </script>';
    exit();
    } else{
    ...
    <option value="<?php echo $row['retailer_id'];?>"><?php echo $row['retailer_name']; ?></option>
